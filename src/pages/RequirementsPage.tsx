...
import { CompassPrioritySelect } from "@/components/shared/LinearIcons";
...
export const RequirementsPage = () => {
  ...
  const { actions: linearActions, labels: linearLabels } = useLinear();
  ...
  const pendingReqs = data.requirements.filter((r) => r.status === "pending").sort((a, b) => (a.order || 0) - (b.order || 0));
  ...
  // Applied filters
  const filteredReqs = pendingReqs
    .filter((req) => {
      // Search
      if (searchQuery) {
  ...
  const handleConfirm = async () => {
    if (confirmConfig.type === "delete" && confirmConfig.id) {
      ...
      const req = data.requirements.find(r => r.id === confirmConfig.id);
      if (!req) return;
      try {
        const categoryLabel = linearLabels.find(l => l.name.toLowerCase() === req.category.toLowerCase());
        await linearActions.createIssue({
          title: req.title,
          description: `[Client: ${req.clientName}]\n\n${req.description}`,
          priority: req.priority,
          labelIds: categoryLabel ? [categoryLabel.id] : undefined,
        });
        actions.markRequirementPromoted(confirmConfig.id);
      } catch (err) {
        console.error('Failed to create Linear issue:', err);
        alert('Failed to create ticket in Linear. Please try again.');
      }
    }
  };
  ...
  return (
    <div className="flex flex-col">
        ...
        <PageToolbar
          ...
          filters={
            <FilterPopover
              ...
              activeCount={(priorityFilter !== "all" ? 1 : 0) + (categoryFilter !== "all" ? 1 : 0) + (assigneeFilter !== "all" ? 1 : 0)}
              onReset={() => {
              ...
              <div className="space-y-4">
                <div className="space-y-1.5">
                  <Select value={assigneeFilter} onValueChange={setAssigneeFilter}>
                    <SelectTrigger className="h-9 w-full">
                      <SelectValue>{assigneeFilter === "all" ? "All Organizations" : null}</SelectValue>
                    </SelectTrigger>
                ...
                <div className="space-y-1.5">
                  <Select value={priorityFilter} onValueChange={setPriorityFilter}>
                    <SelectTrigger className="h-9 w-full">
                      <SelectValue>{priorityFilter === "all" ? "All Priorities" : null}</SelectValue>
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                    <SelectTrigger className="h-9 w-full">
                      <SelectValue>{categoryFilter === "all" ? "All Categories" : null}</SelectValue>
                    </SelectTrigger>
                    <SelectContent>
      ...
      <div className="flex flex-col space-y-3">
        {filteredReqs.length === 0 ? (
          <EmptyState
            ...
            action={
              <Button variant="link" onClick={() => setIsModalOpen(true)} className="text-primary">
                Add Requirement
              </Button>
            }
          />
              ...
              <div className="overflow-x-auto">
                <div className="min-w-[1000px] flex border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900/50">
                  {["id", "title", "category", "priority", "organizations", "reportedBy", "comments"].map((key) => {
                    const label = key === "id" ? "ID" : key === "organizations" ? "Organizations" : key === "reportedBy" ? "Reported By" : key.charAt(0).toUpperCase() + key.slice(1);

                    if (key === "comments") {
                      return (
                        <div key={key} className="w-28 shrink-0 border-r border-zinc-200 dark:border-zinc-800 p-2 font-medium text-zinc-500 text-center">
                          Comments
                        </div>
                      );
                    }

                    const widthClass = key === "id" ? "w-16" : key === "category" ? "w-24" : key === "priority" ? "w-24" : key === "organizations" ? "w-32" : key === "reportedBy" ? "w-32" : "flex-1 min-w-[300px]";

                    return (
                      <div key={key} className={`${widthClass} shrink-0 border-r border-zinc-200 dark:border-zinc-800 p-2 font-medium pl-3 text-zinc-500 cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors flex items-center ${key === "id" ? "justify-center" : "justify-between"} group select-none relative`} onClick={() => handleSort(key)}>
                        <div className="flex items-center gap-1">
                          <motion.span layout>{label}</motion.span>
                          <AnimatePresence mode="wait">
                            {sortConfig?.key === key && (
                              <motion.div key={sortConfig.direction} initial={{ opacity: 0, scale: 0.5, rotate: -180, marginLeft: 0 }} animate={{ opacity: 1, scale: 1, rotate: 0, marginLeft: 2 }} exit={{ opacity: 0, scale: 0.5, rotate: 180, marginLeft: 0 }} transition={{ duration: 0.2 }} layout>
                        ...
                        {filteredReqs.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((req, i) => {
                          ...
                          return (
                            <Draggable key={req.id} draggableId={req.id} index={i}>
                              {(provided, snapshot) => (
                                <div ref={provided.innerRef} {...provided.draggableProps} style={{ ...provided.draggableProps.style }} className={`min-w-[1000px] border-b border-zinc-100 dark:border-zinc-800 last:border-0 ${snapshot.isDragging ? "bg-white dark:bg-zinc-900 shadow-xl ring-1 ring-zinc-200 dark:ring-zinc-800 opacity-90 z-20" : ""}`}>
                                  <div className="group flex hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors">
                                    <div className="w-16 shrink-0 border-r border-zinc-100 dark:border-zinc-800 p-2 text-center text-zinc-400 font-mono text-xs flex items-center justify-between px-3">
                                      <div {...provided.dragHandleProps} className="cursor-grab active:cursor-grabbing hover:text-zinc-600 transition-colors mr-2">
                                        <GripVertical size={14} />
                                      </div>
                                      <span>{(currentPage - 1) * itemsPerPage + i + 1}</span>
                                      {req.description && (
                                        <button onClick={(e) => toggleExpand(req.id, e)} className="p-0.5 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-sm transition-colors text-zinc-400 hover:text-zinc-600 dark:text-zinc-500 dark:hover:text-zinc-300 ml-1">
                                          <ChevronDown size={12} className={`transition-transform duration-200 ${isExpanded ? "rotate-180" : ""}`} />
                                        </button>
                                      )}
                                    </div>
                                    <div
                                      className="flex-1 min-w-[300px] border-r border-zinc-100 dark:border-zinc-800 p-2 pl-3 font-medium text-zinc-700 dark:text-zinc-200 truncate cursor-pointer hover:underline"
                                      onClick={() => {
                                        setEditingRequirement(req);
                                        setEditingRequirementTab("details");
                                        setIsModalOpen(true);
                                      }}
                                    >
                                    ...
                                    <div className="w-24 shrink-0 border-r border-zinc-100 dark:border-zinc-800 p-2 pl-3 capitalize text-zinc-500">
                                      <div className="flex items-center h-full" onClick={(e) => e.stopPropagation()}>
                                        <Select value={req.category} onValueChange={(val) => actions.updateRequirement(req.id, { category: val as any })}>
                                          <SelectTrigger className="h-full w-full border-none shadow-none bg-transparent p-0 capitalize text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 focus:ring-0 [&>svg:last-child]:hidden justify-start">{req.category}</SelectTrigger>
                                          <SelectContent>
                                            <SelectItem value="feature">Feature</SelectItem>
                                            <SelectItem value="improvement">Improvement</SelectItem>
                                            <SelectItem value="bug">Bug</SelectItem>
                                    ...
                                    <div className="w-24 shrink-0 border-r border-zinc-100 dark:border-zinc-800 p-2 pl-3">
                                      <div className="flex items-center h-full" onClick={(e) => e.stopPropagation()}>
                                        <CompassPrioritySelect value={req.priority} onValueChange={(val) => actions.updateRequirement(req.id, { priority: val as any })} />
                                      </div>
                                    </div>
                                    <div className="w-32 shrink-0 border-r border-zinc-100 dark:border-zinc-800 p-1 pl-1 flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
                                      <MultiSelectDropdown
                                        options={[{ id: "all", name: "All Organizations" }, ...data.organizations.map((o) => ({ id: o.id, name: o.name }))]}
                                        ...
                                        triggerClassName="h-8 w-fit min-w-[20px] px-1 justify-center font-normal hover:bg-zinc-100 dark:hover:bg-zinc-800/50 !border-none !ring-0 shadow-none rounded-full bg-transparent"
                                    ...
                                    <div className="w-32 shrink-0 border-r border-zinc-100 dark:border-zinc-800 p-2 pl-3 text-zinc-600 dark:text-zinc-400 truncate">
                                      <div className="flex items-center h-full w-full" onClick={(e) => e.stopPropagation()}>
                                        <Select value={req.reportedBy && data.users.find((u) => u.id === req.reportedBy || u.name === req.reportedBy) ? (data.users.find((u) => u.id === req.reportedBy || u.name === req.reportedBy)?.id ?? "unassigned") : "unassigned"} onValueChange={(val) => actions.updateRequirement(req.id, { reportedBy: val === "unassigned" ? undefined : val })}>
                                              ...
                                              if (reporter) {
                                                return (
                                                  <div className="flex items-center gap-2 pointer-events-none">
                                                    <div className="w-5 h-5 rounded-full bg-primary/10 text-primary flex items-center justify-center text-[10px] font-bold shrink-0">{reporter.name.charAt(0)}</div>
                                            ...
                                            {data.users.map((u) => (
                                              <SelectItem key={u.id} value={u.id}>
                                                <div className="flex items-center gap-2">
                                                  <div className="w-5 h-5 rounded-full bg-primary/10 text-primary flex items-center justify-center text-[10px] font-bold border border-primary/20">{u.name.charAt(0)}</div>
                                                  {u.name}
                                                </div>
                                              </SelectItem>
                                            ))}
                                      ...
                                      {req.comments && req.comments.length > 0 ? (
                                        <button
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            setEditingRequirement(req);
                                            setEditingRequirementTab("comments");
                                            setIsModalOpen(true);
                                          }}
                                  ...
                                  <AnimatePresence>
                                    {isExpanded && req.description && (
                                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden bg-zinc-50/50 dark:bg-zinc-900/30 border-t border-zinc-100 dark:border-zinc-800">
            ...
            {filteredReqs.length > itemsPerPage && (
              <div className="flex items-center justify-between pt-2">
                ...
                <div className="flex gap-1">
                  ...
                  <Button variant="outline" size="sm" disabled={currentPage * itemsPerPage >= filteredReqs.length} onClick={() => setCurrentPage((p) => p + 1)} className="h-8 w-8 p-0">
      ...
      <ConfirmDialog isOpen={confirmConfig.isOpen} onClose={() => setConfirmConfig({ ...confirmConfig, isOpen: false })} onConfirm={handleConfirm} title={confirmConfig.title} description={confirmConfig.description} variant={confirmConfig.type === "delete" ? "danger" : "success"} confirmText={confirmConfig.type === "delete" ? "Delete" : "Promote"} />
    </div>
  );
};
    ...
    <Modal
      ...
      title={
        <div className="flex items-center gap-3">
          <div className="p-2 bg-purple-100 dark:bg-purple-900/30 rounded-full text-purple-600 dark:text-purple-500">{initialData ? <Pencil size={20} /> : <ClipboardList size={20} />}</div>
    ...
    >
      {initialData && (
        <div className="flex gap-4 border-b border-zinc-100 dark:border-zinc-800 mb-4">
          <button onClick={() => setActiveTab("details")} className={`pb-2 text-sm font-medium border-b-2 transition-colors ${activeTab === "details" ? "border-primary text-primary" : "border-transparent text-zinc-500 hover:text-zinc-700"}`}>
            Details
          </button>
          <button onClick={() => setActiveTab("comments")} className={`pb-2 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === "comments" ? "border-primary text-primary" : "border-transparent text-zinc-500 hover:text-zinc-700"}`}>
            Comments
            <span className="bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 text-[10px] px-1.5 py-0.5 rounded-full">{currentRequirement?.comments?.length || 0}</span>
          </button>
        </div>
      )}
      ...
      {activeTab === "details" ? (
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input required placeholder="e.g. Export Reports to PDF" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5 p-1">
              <label className="text-xs font-bold uppercase text-primary mb-1 block">Category</label>
              <Select value={formData.category} onValueChange={(val) => setFormData({ ...formData, category: val as TicketType })}>
          ...
          <div className="space-y-1.5">
            ...
            <Select value={formData.reportedBy} onValueChange={(val) => setFormData({ ...formData, reportedBy: val })}>
              <SelectTrigger>
          ...
          <div className="space-y-1.5">
            ...
            <Textarea placeholder="Detailed requirements from the client..." className="min-h-[120px]" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} />
          </div>
        ...
        <div className="h-[400px] flex flex-col">
          <div className="flex-1 overflow-y-auto space-y-6 mb-4 pr-1">
            ...
            {currentRequirement?.comments?.map((comment) => (
              <div key={comment.id} className="flex gap-3 animate-in slide-in-from-bottom-2 fade-in duration-300">
                <div className="w-8 h-8 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center text-xs font-bold text-zinc-500 shrink-0">{comment.author.charAt(0).toUpperCase()}</div>
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-zinc-900 dark:text-zinc-100">{comment.author}</span>
                    <span className="text-xs text-zinc-400">
                      {new Date(comment.createdAt).toLocaleDateString()} at {new Date(comment.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", hour12: true })}
                    </span>
                  </div>
                  <p className="text-sm text-zinc-600 dark:text-zinc-300 leading-relaxed bg-zinc-50 dark:bg-zinc-900 p-2 rounded-md rounded-tl-none">{comment.text}</p>
                </div>
              </div>
            ))}
          ...
          <div className="flex gap-2 items-end pt-2 border-t border-zinc-100 dark:border-zinc-800">
            <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-xs font-bold text-primary shrink-0 mb-1">U</div>
