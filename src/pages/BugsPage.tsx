...
import { Bug as BugIcon, Plus, Pencil, ArrowRight, GripVertical, Paintbrush, XCircle, AlertCircle, AlertTriangle, MinusCircle, Trash2, ChevronLeft, ChevronRight, ChevronUp, ChevronDown } from "lucide-react";
...
export const BugsPage = () => {
  ...
  const handleConfirm = async () => {
    if (confirmConfig.type === "delete" && confirmConfig.id) {
      ...
      const bug = data.bugs.find(b => b.id === confirmConfig.id);
      if (!bug) return;
      try {
        const bugLabel = linearLabels.find(l => l.name.toLowerCase() === 'bug');
        ...
        actions.markBugPromoted(confirmConfig.id);
      } catch (err) {
    ...
    <div className="flex flex-col h-full">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex bg-zinc-100 dark:bg-zinc-800 rounded-lg p-0.5 h-8 relative">
                {(["active", "closed", "all"] as const).map((status) => (
                  <button key={status} onClick={() => setStatusFilter(status)} className={cn("relative px-3 text-xs font-medium rounded-md transition-colors z-10 h-full capitalize", statusFilter === status ? "text-foreground" : "text-zinc-500 hover:text-zinc-700")}>
                    {statusFilter === status && <motion.div layoutId="status-filter-pill" className="absolute inset-0 bg-white dark:bg-zinc-700 shadow-sm rounded-md ring-1 ring-black/5 dark:ring-white/10" transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />}
                ...
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <Select value={severityFilter} onValueChange={setSeverityFilter}>
                      <SelectTrigger className="h-9 w-full">
                        <SelectValue>{severityFilter === "all" ? "All Severities" : null}</SelectValue>
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-1.5">
                    <Select value={platformFilter} onValueChange={setPlatformFilter}>
                      <SelectTrigger className="h-9 w-full">
                        <SelectValue>{platformFilter === "all" ? "All Platforms" : null}</SelectValue>
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-1.5">
                    <Select value={assigneeFilter} onValueChange={setAssigneeFilter}>
                      <SelectTrigger className="h-9 w-full">
                        <SelectValue>{assigneeFilter === "all" ? "All Assignees" : null}</SelectValue>
                      </SelectTrigger>
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        {filteredBugs.length === 0 ? (
          <EmptyState icon={BugIcon} title="No bugs found" description={statusFilter === "active" ? "Great job! No active bugs." : "No bugs match your filters."} action={<Button onClick={() => setIsModalOpen(true)}>Report Bug</Button>} />
        ) : (
          <>
              ...
              <div className="min-w-[1000px]">
                ...
                <div className="flex font-medium text-zinc-500 border-b bg-zinc-50/50 dark:bg-zinc-900/50 sticky top-0 z-10 w-full px-1">
                  {["id", "severity", "title", "layer", "assignee", "status"].map((key) => {
                    const label = key === "id" ? "ID" : key === "title" ? "Bug Details" : key === "assignee" ? "Assigned To" : key === "layer" ? "Stack" : key.charAt(0).toUpperCase() + key.slice(1);

                    const widthClass = key === "id" ? "w-20" : key === "severity" ? "w-28" : key === "title" ? "flex-1 min-w-[300px]" : key === "layer" ? "w-20" : key === "assignee" ? "w-32" : "w-28";

                    return (
                      <div key={key} className={`${widthClass} p-2 border-r flex items-center ${key === "title" ? "pl-3" : "justify-center"} cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-colors select-none`} onClick={() => handleSort(key as keyof Bug)}>
                        ...
                        {filteredBugs.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((bug, idx) => {
                          ...
                          return (
                            <Draggable key={bug.id} draggableId={bug.id} index={idx}>
                              {(provided, snapshot) => (
                                <div ref={provided.innerRef} {...provided.draggableProps} style={{ ...provided.draggableProps.style }} className={cn("flex group border-b last:border-0 hover:bg-zinc-50 dark:hover:bg-zinc-900/40 transition-colors w-full px-1", snapshot.isDragging && "bg-white dark:bg-zinc-900 shadow-xl ring-1 ring-zinc-200 dark:ring-zinc-800 opacity-90 z-20")}>
                                  <div className="w-20 border-r flex items-center px-4 gap-4">
                                    <div className={cn("w-4 h-4 flex items-center justify-center transition-colors text-zinc-400 group-hover:text-zinc-600 cursor-grab active:cursor-grabbing")} {...provided.dragHandleProps} title="Drag to reorder">
                                  ...
                                  <div className="w-28 p-2 flex justify-center items-center border-r" onClick={(e) => e.stopPropagation()}>
                                    <Select value={bug.severity} onValueChange={(val) => actions.updateBug(bug.id, { severity: val as any })} className="flex justify-center">
                                  ...
                                  >
                                    <div className="font-medium text-zinc-700 dark:text-zinc-200 truncate pr-4">{bug.title}</div>
                                  </div>
                                  <div className="w-20 p-2 border-r flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
                                    <Select value={bug.layer || "none"} onValueChange={(val) => actions.updateBug(bug.id, { layer: val === "none" ? undefined : (val as any) })} className="flex justify-center">
                                      <SelectTrigger className="h-auto w-fit p-0 border-none shadow-none bg-transparent hover:bg-zinc-100 dark:hover:bg-zinc-800 focus:ring-0 flex justify-center [&>svg:last-child]:hidden rounded-full">
                                        {bug.layer ? (
                                  ...
                                  <div className="w-32 p-2 border-r flex items-center justify-center px-4" onClick={(e) => e.stopPropagation()}>
                                    <Select value={bug.assignee || "unassigned"} onValueChange={(val) => actions.updateBug(bug.id, { assignee: val === "unassigned" ? undefined : val })}>
                                      <SelectTrigger className="h-full w-full p-0 border-none shadow-none bg-transparent hover:bg-zinc-100 dark:hover:bg-zinc-800 focus:ring-0 flex items-center justify-center overflow-hidden [&>svg:last-child]:hidden">
                                        {assignee ? (
                                          <div className="flex items-center gap-2 pointer-events-none">
                                            <div className="w-6 h-6 rounded-full bg-primary/10 text-primary flex items-center justify-center text-[10px] font-bold shrink-0">{assignee.name.charAt(0)}</div>
                                  ...
                                  <div className="w-28 p-2 border-r flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
                                    <Select value={bug.status} onValueChange={(val) => actions.updateBug(bug.id, { status: val as any })} className="flex justify-center">
                                      <SelectTrigger className="h-auto w-fit p-0 border-none shadow-none bg-transparent hover:bg-zinc-100 dark:hover:bg-zinc-800 focus:ring-0 flex justify-center [&>svg:last-child]:hidden rounded-md">
                                        <span
                                          className={`inline-flex items-center px-2 py-1 rounded-md text-[9px] font-bold uppercase whitespace-nowrap border pointer-events-none
      ...
      <BugModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} initialData={editingBug} />
      ...
      <ConfirmDialog isOpen={confirmConfig.isOpen} onClose={() => setConfirmConfig({ isOpen: false, type: null, id: null })} onConfirm={handleConfirm} title={confirmConfig.type === "delete" ? "Delete Bug?" : "Promote to Ticket?"} description={confirmConfig.type === "delete" ? "This cannot be undone." : "This will create a backlog ticket and mark this bug as In Progress."} variant={confirmConfig.type === "delete" ? "danger" : "success"} />
    </div>
  );
};
